...
def process_payment(ccn, mm, yy, cvc):
    ...
    try:
        ...
        if 'id' not in apx:
            return {"status": "error", "message": apx.get('error', {}).get('message', 'Unknown error')}
        ...
        if "failureType" in apx1:
            return {"status": "error", "message": f"{ccn}|{mm}|{yy}|{cvc} --> {apx1['failureType']}"}
        else:
            return {"status": "success", "message": f"{ccn}|{mm}|{yy}|{cvc} --> PAYMENT_SUCCESS"}
